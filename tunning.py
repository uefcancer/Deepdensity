...
## hyperparmetres
learning_rates = [0.0001, 0.00009, 0.00008]
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='dataset_name', type=str, help='Mammogram dataset names')
...
for lr, batch_size, optimizer_class in hyperparameter_combinations:
    ...
    with open(config['logs_file_path'], 'a+') as logs_file:
        ...
        for i in range(0, config['num_epochs']):
            ...
            train_logs = train_epoch.run(DataLoader(train_dataset, shuffle=True, batch_size = config['train_batch_size'], num_workers=config['num_workers']))
            valid_logs = valid_epoch.run(DataLoader(valid_dataset, shuffle=True, batch_size=config['valid_batch_size'], num_workers=config['num_workers']))
